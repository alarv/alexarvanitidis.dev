export const metadata = {
  title:
    "The 5 worst bugs I've seen on production - #2 the infinite crawler | Alex Arvanitidis's Blog",
  description:
...
Last but not least: Always have a kill switch for a loop. In any exceptional case the loop could go on forever, so there should be a hard limit such as a hard cap that kills the loop and logs an error, instead of producing a bug that is difficult to solve on a prod environment.
